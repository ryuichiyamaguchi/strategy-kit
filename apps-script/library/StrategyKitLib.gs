/**
 * STRATEGY-KIT Library — マスター GAS（v0.11.0）
 *
 * このファイルは「ライブラリとしてデプロイされる側」のロジック本体。
 * 受講者の GAS には貼らない（受講者は shim/Code.gs だけ貼る）。
 *
 * 設計方針:
 *   - PropertiesService を引数として `props` オブジェクトで受け取る
 *     （ライブラリの ScriptProperties にアクセスしないため）
 *   - DocumentApp.getActiveDocument() は呼び出し元コンテキストで動くため
 *     ライブラリ内でも問題なく使える
 *   - onOpen / doPost / doGet / メニューハンドラは shim 側で定義する
 *   - ライブラリ側は `ui*()` 系のダイアログ実装と `route()` を提供
 *
 * 公開インターフェース（shim から呼ばれる関数）:
 *   - getVersionInfo()                                 → version/productLine/schemaVersion
 *   - route(e, props)                                  → Web App リクエストルータ
 ...
 *
 * 受講者プロパティ仕様（props オブジェクト）:
 *   - props.SK_DOC_ID         : マスタードキュメント ID
 *   - props.SK_DRAFT_DOC_ID   : DRAFT Doc ID
 *   - props.SK_TEMPLATE_DOC_ID: テンプレート原本 Doc ID
 *   - props.GEMINI_API_KEY    : Gemini API キー
 *   - props.setProperty(k, v) : プロパティ書込
 *   - props.getProperty(k)    : プロパティ読出
 */

const SK_VERSION = '0.11.2';
...
/**
 * shim から呼ばれる version 情報取得（公開 API）。
 */
function getVersionInfo() {
  return _versionPayload_();
}
...
function _requireProps_(props) {
  if (!props || typeof props.getProperty !== 'function') {
    throw new Error('props オブジェクトが不正です（shim/Code.gs の _props_() を確認してください）');
  }
  return props;
}

// =====================================================
// UI ダイアログ系（shim メニューから呼ばれる）
// =====================================================

/**
 * テンプレート原本 Doc ID をユーザに入力させて props に保存する。
 */
function uiSetupTemplate(props) {
  _requireProps_(props);
  const ui = DocumentApp.getUi();
  const r = ui.prompt(
    'STRATEGY-KIT v0.11 セットアップ',
  ...
  try {
    ...
    props.setProperty(SK_TEMPLATE_DOC_ID_KEY, docId);
    ui.alert(
      'セットアップ完了',
      'テンプレート原本を登録しました\n\nファイル名: ' + name + '\nDoc ID: ' + docId,
      ui.ButtonSet.OK
...
/**
 * 業種プリセットを指定して新規マスタードキュメントを生成（UIフロー）。
 */
function uiCreateMaster(presetId, props) {
  _requireProps_(props);
  const ui = DocumentApp.getUi();
  ...
  props.setProperty('SK_DOC_ID', result.docId);
 ...
 * §99 決定ログ追記ダイアログ。
 */
function uiAppendDecision(props) {
  _requireProps_(props);
  const ui = DocumentApp.getUi();
  ...
  appendDecisionEntry_(decision, reason, action, props);
 ...
 * 章末タイムスタンプ更新ダイアログ。
 */
function uiUpdateTimestamps(props) {
  _requireProps_(props);
  const ui = DocumentApp.getUi();
  const r = ui.prompt(
    '章末タイムスタンプ',
  ...
  const updated = updateTimestamp_(target, props);
 ...
 * §N 要点版追記ダイアログ。
 */
function uiAppendSectionSummary(props) {
  _requireProps_(props);
  const ui = DocumentApp.getUi();
  const r1 = ui.prompt(
    '§N 要点版',
  ...
  try {
    const result = appendSectionSummary_(sectionNo, summaryText, undefined, props);
    ui.alert(
      'STRATEGY-KIT',
...
function uiShowHelp(props) {
  ...
  const html = HtmlService.createHtmlOutput(
    '<div style="font-family:-apple-system,sans-serif;font-size:13px;line-height:1.6;padding:8px;">' +
      ...
      '</ul>' +
      '<h2 style="font-size:15px;margin:12px 0 8px;">v0.11 の主要変更</h2>' +
      '<ul>' +
        '<li><strong>ロジックがライブラリ化</strong> — 受講者は薄い shim を1回貼るだけ。本体更新は山口さん側で完結</li>' +
        '<li>章構造の正本は <code>template_v3.0.md</code> に維持</li>' +
      '</ul>' +
 ...
 * @param {object} props 受講者の props オブジェクト
 * @return {{ docId:string, docUrl:string, name:string }}
 */
function createMasterFromTemplate_(presetId, formInputs, props) {
  _requireProps_(props);
  const templateId = props.getProperty(SK_TEMPLATE_DOC_ID_KEY);
  if (!templateId) {
    throw new Error('テンプレート原本未登録。先に setupTemplate を実行してください');
  }
...
function appendDecisionEntry_(decision, reason, action, props) {
  _requireProps_(props);
  const doc = _getDoc_(props);
...
function updateTimestamp_(targetSection, props) {
  _requireProps_(props);
  const doc = _getDoc_(props);
  const body = doc.getBody();
  const today = _today_();
 ...
 */
function appendSectionSummary_(sectionNo, summaryText, opt_docId, props) {
  _requireProps_(props);
  const doc = opt_docId
...
function getSectionSummary_(sectionNo, opt_docId, props) {
  _requireProps_(props);
  const doc = opt_docId
    ? DocumentApp.openById(opt_docId)
    : _getDoc_(props);
...
// =====================================================
// Web App ルーター（shim の doPost / doGet から呼ばれる）
// =====================================================

/**
 * 受講者の doPost / doGet からそのまま委譲される Web App ルータ。
 * @param {object} e Apps Script Web App リクエストオブジェクト
 * @param {object} props 受講者の props オブジェクト
 */
// v0.11 Lv2: docId 許可リストチェック対象外の action
//   ping/health: 識別情報のみ
//   diagnoseSetup: セットアップ診断（docId 受け取らない）
//   setupTemplate/getTemplateInfo: テンプレ初期登録系（docId 不要）
//   setMasterDoc/setDraftDoc: docId を ScriptProperties に登録するアクション本体（チェックしたら登録できない）
//   geminiProxy: docId 不要
const SK_DOC_ID_BYPASS_ACTIONS = {
  ping: true,
  diagnoseSetup: true,
  setupTemplate: true,
...
/**
 * v0.11 Lv2 セキュリティ: 受け取った docId が ScriptProperties に登録された
 * SK_DOC_ID / SK_DRAFT_DOC_ID / SK_TEMPLATE_DOC_ID のいずれかと一致するか検証する。
 * 一致しない docId 注入をブロックして、受講者と共有された他人の Doc が
 * 勝手に書き換えられるリスクを防ぐ。
 */
function _assertDocAccess_(payload, action, props) {
  if (SK_DOC_ID_BYPASS_ACTIONS[action]) return;
  const receivedDocId = payload && (payload.docId || payload.draftDocId);
  if (!receivedDocId) return; // payload に docId が無ければハンドラ側で props.SK_DOC_ID を解決

  const allowed = ['SK_DOC_ID', 'SK_DRAFT_DOC_ID', 'SK_TEMPLATE_DOC_ID']
    .map(function (k) { return props.getProperty(k); })
    .filter(Boolean);

  if (allowed.length === 0) return; // 初期セットアップ中（プロパティ未登録）はバイパス

  if (allowed.indexOf(receivedDocId) === -1) {
    throw new Error(
      'Unauthorized docId: 許可されていない Doc ID への操作はブロックされました（v0.11 Lv2 セキュリティ）。' +
      'この Doc は受講者本人のマスタードキュメント／DRAFT／テンプレート原本のいずれにも登録されていません。'
    );
  }
}

function route(e, props) {
  _requireProps_(props);

  // GET は簡易ヘルスチェック（認証不要・プローブ可能なまま）
  if (!e || !e.postData || !e.postData.contents) {
    return _jsonResponse_(Object.assign({ ok: true, action: 'health' }, _versionPayload_()));
  }
  ...
  try {
    ...
    return _jsonResponse_({ ok: false, error: 'invalid JSON: ' + err.message });
  }
  const action = raw.__action || raw.action || (e && e.parameter && e.parameter.action) || 'ping';
  const payload = raw.payload && typeof raw.payload === 'object' ? raw.payload : raw;

  try {
    // v0.11.2: 任意トークン認証（SK_API_TOKEN が設定されていれば必須）
    _requireToken_(raw, e, props);

    // v0.11 Lv2: docId 許可リストチェック（任意 docId 注入を防ぐ）
    _assertDocAccess_(payload, action, props);

    switch (action) {
      case 'ping':
        return _jsonResponse_(handle_ping_(payload, props));
      case 'diagnoseSetup':
        return _jsonResponse_(handle_diagnoseSetup_(payload, props));
      case 'getSection':
        return _jsonResponse_(handle_getSection_(payload, props));
      case 'getAllSections':
        return _jsonResponse_(handle_getAllSections_(payload, props));
      case 'getProgress':
        return _jsonResponse_(handle_getProgress_(payload, props));
      case 'saveResearch':
        return _jsonResponse_(handle_saveResearch_(payload, props));
      case 'appendDecision':
        return _jsonResponse_(handle_appendDecision_(payload, props));
      case 'appendMeta':
        return _jsonResponse_(handle_appendMeta_(payload, props));
      case 'updateTimestamp':
        return _jsonResponse_(handle_updateTimestamp_(payload, props));
      case 'geminiProxy':
        return _jsonResponse_(handle_geminiProxy_(payload, props));
      case 'listResearchFiles':
        return _jsonResponse_(handle_listResearchFiles_(payload, props));
      case 'createDraftDoc':
        return _jsonResponse_(handle_createDraftDoc_(payload, props));
      case 'appendDraftSection':
        return _jsonResponse_(handle_appendDraftSection_(payload, props));
      case 'getDraftInfo':
        return _jsonResponse_(handle_getDraftInfo_(payload, props));
      case 'cleanupDraft':
        return _jsonResponse_(handle_cleanupDraft_(payload, props));
      case 'generateExecutiveSummary':
        return _jsonResponse_(handle_generateExecutiveSummary_(payload, props));
      case 'setMasterDoc':
        return _jsonResponse_(handle_setMasterDoc_(payload, props));
      case 'getMasterDocInfo':
        return _jsonResponse_(handle_getMasterDocInfo_(payload, props));
      case 'getDraftProgress':
        return _jsonResponse_(handle_getDraftProgress_(payload, props));
      case 'setDraftDoc':
        return _jsonResponse_(handle_setDraftDoc_(payload, props));

      // v0.10/v0.11 新規
      case 'setupTemplate':
        return _jsonResponse_(handle_setupTemplate_(payload, props));
      case 'getTemplateInfo':
        return _jsonResponse_(handle_getTemplateInfo_(payload, props));
      case 'createMasterFromTemplate':
        return _jsonResponse_(handle_createMasterFromTemplate_(payload, props));
      case 'appendSectionSummary':
        return _jsonResponse_(handle_appendSectionSummary_(payload, props));
      case 'getSectionSummary':
        return _jsonResponse_(handle_getSectionSummary_(payload, props));

      default:
        return _jsonResponse_({ ok: false, error: 'unknown action: ' + action });
    }
  } catch (err) {
    return _jsonResponse_({ ok: false, error: err.message, stack: String(err.stack || '').slice(0, 400) });
  }
}

// =====================================================
// 任意トークン認証（v0.11.2 で導入・後方互換）
// =====================================================
//
// 受講者がスクリプトプロパティ "SK_API_TOKEN" を設定した場合のみ有効化される。
// 未設定なら認証なし（v0.11.1 までの動作と同じ）。
//
// クライアントは POST 本体の JSON のトップレベル `token` フィールドに同じ値を
// 入れて送る（拡張機能側 sk-gas-client.js で自動付与）。GET の URL パラメータ
// `?token=...` も受け付けるが、Web App URL と一緒にサーバログへ残るため
// 拡張機能からは POST 本体経由を優先する。
//
// 一致しない／空のときは UNAUTHORIZED を投げて route() の catch が JSON エラー
// を返す。攻撃者には「キーが必要」以上の情報を返さない。
function _requireToken_(raw, e, props) {
  const expected = props.getProperty('SK_API_TOKEN');
  if (!expected) return; // トークン未設定 = 認証なし（後方互換）
  const got =
    (raw && (raw.token || raw.__token)) ||
    (raw && raw.payload && raw.payload.token) ||
...
function _getDraftDoc_(props) {
  _requireProps_(props);
  const draftId = props.getProperty('SK_DRAFT_DOC_ID');
  if (!draftId) {
...
// =====================================================
// Web App ハンドラ群（props 引数化）
// =====================================================

function handle_ping_(body, props) {
  const doc = _getDoc_(props);
  const templateId = props.getProperty(SK_TEMPLATE_DOC_ID_KEY);
...
function handle_diagnoseSetup_(body, props) {
  ...
  let tplStatus = 'warn';
  let tplLabel = '未登録';
  let tplDetail = 'テンプレート原本 Doc が未登録です。setupTemplate を実行してください。';
  ...
  if (templateId) {
    try {
      ...
      tplRecovery = 'setupTemplate を再実行して正しい Doc ID を登録してください';
    }
  }
  checks.push({
    id: 'template-doc',
  ...
  if (geminiApiKey && runGeminiProbe) {
    const probe = _geminiCall_('OK', SK_GEMINI_DEFAULT_MODEL, 0, geminiApiKey);
...
// 章本文の取得
// -----------------------------------------------------
function _getSections_(doc, props) {
  if (!doc) doc = _getDoc_(props);
  const body = doc.getBody();
  const num = body.getNumChildren();
  const sections = [];
  let current = null;
...
function handle_getSection_(body, props) {
  ...
  let doc;
  try {
    doc = source === 'draft' ? _getDraftDoc_(props) : _getDoc_(props);
  } catch (e) {
    return { ok: false, error: e.message };
  }
  const sections = _getSections_(doc, props);
...
function handle_getAllSections_(body, props) {
  ...
  return { ok: true, source: source, sections: _getSections_(doc, props) };
}

function handle_getProgress_(body, props) {
  const sections = _getSections_(undefined, props);
...
// リサーチ保存
// -----------------------------------------------------
function handle_saveResearch_(body, props) {
...
function handle_listResearchFiles_(body, props) {
  const doc = _getDoc_(props);
  const docFile = DriveApp.getFileById(doc.getId());
  const parents = docFile.getParents();
  const parent = parents.hasNext() ? parents.next() : DriveApp.getRootFolder();
...
// 決定ログ追記
// -----------------------------------------------------
function handle_appendDecision_(body, props) {
...
// §-1 案件メタ情報の差し替え
// -----------------------------------------------------
function handle_appendMeta_(body, props) {
  ...
  // フォールバック: テンプレート方式でない（プレースホルダなし）→ 旧 v0.9.x 互換
...
function handle_updateTimestamp_(body, props) {
...
// Gemini プロキシ
// -----------------------------------------------------
function handle_geminiProxy_(body, props) {
  const apiKey = props.getProperty('GEMINI_API_KEY');
  if (!apiKey) return { ok: false, error: 'GEMINI_API_KEY が未設定です' };
  ...
  for (let mi = 0; mi < modelChain.length; mi++) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      const text = _geminiCall_(prompt, tryModel, temperature, apiKey);
...
function handle_createDraftDoc_(body, props) {
  let masterDoc;
  try {
    masterDoc = _getDoc_(props);
  } catch (e) {
  ...
  props.setProperty('SK_DRAFT_DOC_ID', draftId);
...
function handle_appendDraftSection_(body, props) {
  const draftId = props.getProperty('SK_DRAFT_DOC_ID');
...
// =====================================================
// DRAFT整形（[CLEAN] Doc 出力）
// =====================================================
function handle_cleanupDraft_(body, props) {
  const draftId = props.getProperty('SK_DRAFT_DOC_ID');
...
function handle_setMasterDoc_(body, props) {
...
function handle_getMasterDocInfo_(body, props) {
  const docId = props.getProperty('SK_DOC_ID');
  if (!docId) {
...
// Executive Summary
// =====================================================
function handle_generateExecutiveSummary_(body, props) {
  const draftId = props.getProperty('SK_DRAFT_DOC_ID');
  if (!draftId) return { ok: false, error: 'DRAFT 未作成' };
  const apiKey = props.getProperty('GEMINI_API_KEY');
  if (!apiKey) return { ok: false, error: 'GEMINI_API_KEY 未設定' };
...
function handle_getDraftInfo_(body, props) {
  const draftId = props.getProperty('SK_DRAFT_DOC_ID');
  if (!draftId) {
    return { ok: true, exists: false };
  }
...
// DRAFT Doc の記入済み章を検出
// -----------------------------------------------------
function handle_getDraftProgress_(body, props) {
  let draft;
  try { draft = _getDraftDoc_(props); } catch (e) { return { ok: false, error: e.message }; }
...
function handle_setDraftDoc_(body, props) {
  const url = String(body.url || '').trim();
  if (!url) return { ok: false, error: 'url is required' };
  ...
  props.setProperty('SK_DRAFT_DOC_ID', docId);
...
function handle_setupTemplate_(body, props) {
...
function handle_getTemplateInfo_(body, props) {
  const templateId = props.getProperty(SK_TEMPLATE_DOC_ID_KEY);
...
function handle_createMasterFromTemplate_(body, props) {
  ...
  let result;
  try {
    result = createMasterFromTemplate_(presetId, inputs, props);
  } catch (e) {
    return { ok: false, error: e.message };
  }
...
function handle_appendSectionSummary_(body, props) {
  ...
  try {
    const result = appendSectionSummary_(sectionNo, summaryText, docId, props);
...
function handle_getSectionSummary_(body, props) {
  const sectionNo = String((body && body.sectionNo) || '').replace(/[^0-9]/g, '');
  if (!sectionNo) return { ok: false, error: 'sectionNo is required' };
  const docId = body && body.docId ? String(body.docId).trim() : '';

  try {
    return getSectionSummary_(sectionNo, docId, props);
  } catch (e) {
    return { ok: false, error: e.message };
  }
}
